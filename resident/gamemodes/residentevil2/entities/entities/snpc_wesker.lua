...
--Model Settings--
ENT.Model = "models/player/slow/amberlyn/re5/wesker/slow.mdl"
...
function ENT:Initialize()
	...
	self:SetHealth(self.health * 2 * GAMEMODE.ZombieData[GetGlobalString("Re2_Difficulty")].Modifier)	
	...
	--Misc--
	self:Precache()
	self.Entity:SetCollisionBounds( Vector(-7,-7,0), Vector(7,7,64) ) 
end
...
function ENT:CustomChaseEnemy()
	...
	if self:IsLineOfSightClear( enemy ) then
		...
		if self:GetRangeTo( enemy ) > self.InitialAttackRange and self:GetRangeTo( enemy ) < 325 then
			...
			if ( self.NextThrow or 0 ) < CurTime() then
				...
				timer.Simple( 0.6, function()
					...
					if self.Attacking then return end
					self:ThrowGrenade( math.random(200, 240) )
					self.Throwing = false
				end)
		...
		if self:GetRangeTo( enemy ) > self.InitialAttackRange and self:GetRangeTo( enemy ) > 85 then
