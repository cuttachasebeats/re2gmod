...
function ENT:GiveWeapon(wep)
	...
	if SERVER  then
	local wep = ents.Create("weapon_minigun2_re")
...
function ENT:ThrowGrenade( velocity )
	...
	local ent = ents.Create("snpc_shambler")
...
function ENT:CustomRegen()
	...
	if ( self.regain or 0 ) < CurTime() then
	
	self:RestartGesture(ACT_GMOD_GESTURE_BOW)
	self.regen = true
...
function ENT:CustomChaseEnemy()
	...
	if self:IsLineOfSightClear( enemy ) then
		...
		if self:GetRangeTo( enemy ) > self.InitialAttackRange and self:GetRangeTo( enemy ) > 120 then
